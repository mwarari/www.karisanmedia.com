...
<?php $count=0; while($featured4postquery->have_posts()) : $featured4postquery->the_post(); ?>
				...
				<?php $thecimage = get_image_for_crop($post->ID,$thumborno=1);
					if (isset($thecimage) && !empty($thecimage)) { ?>
						...
						<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e("Permanent Link to",'KarisanMediaTheme');?> <?php the_title_attribute(); ?>"><img src="<?php echo bloginfo('template_url'); ?>/<?php echo $cropperfile;?>?src=<?php echo $thecimage; ?>&amp;h=75&amp;w=75&amp;zc=1&amp;q=95" alt="<?php the_title(); ?>" /></a>
					<?php
					}
					else
					...
					if(strlen($thetitle) > 60)
					{
						$thetitle=LimitText($thetitle,10,60,"");
						$thetitle.="&raquo;";
					}
