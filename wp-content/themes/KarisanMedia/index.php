...
<div class="entry">
<h1>About Karisan Media Theme</h1>
<p>You are viewing Karisan Media Theme with the news magazine style turned off. Once you turn on the news magazine style from your theme options page the theme should look like the screenshot below. To setup go to the theme options page to select your featured categories and configure your other settings.</p>
...
<p>If you prefer to use this layout leave "Turn On or Off the News Magazine Layout" set to off and turn off this explanation text and screenshot example from the theme options page using the "Turn Off <b>About Karisan Media Theme</b>" option</p>
</div>
<?php } ?>
	...
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			...
			<div class="tex">
			<h2><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent link to','KarisanMediaTheme'); ?> <?php the_title(); ?>"><?php the_title(); ?></a></h2>

			<?php if(get_option($themeoptionsprefix.'_showhideauthordatebyline') == 'on'){ ?>
			<div class="bylineexcerpt">
			<?php _e("Written by",'KarisanMediaTheme');?> <?php the_author(); ?>
			...
			<div class="clear"></div>
			<?php } else { ?>
			<h2 class="h2titleindex"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent link to','KarisanMediaTheme'); ?> <?php the_title(); ?>"><?php the_title(); ?></a></h2>
			<?php if(get_option($themeoptionsprefix.'_showhideauthordatebyline') == 'on'){ ?>
